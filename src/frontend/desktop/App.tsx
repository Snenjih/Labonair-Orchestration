...
function deriveTasks(history: ParsedEvent[]): TaskItem[] {
  ...
  for (const ev of history) {
    if (ev.type === 'tool_call_start') {
      pendingName = ev.toolName;
      pendingArgs = ev.args;
    } else if (ev.type === 'tool_call_end') {
      if (pendingName === 'TaskCreate' && pendingArgs) {
        try {
          const args = JSON.parse(pendingArgs);
          const id = String(args.id ?? `task-${tasks.size}`);
          tasks.set(id, {
            id,
            title: args.title ?? args.task ?? '(task)',
            status: 'pending',
            createdAt: Date.now(),
          });
        } catch { /* ignore */ }
      } else if (pendingName === 'TaskUpdate' && pendingArgs) {
        try {
          const args = JSON.parse(pendingArgs);
          ...
          if (id && tasks.has(id)) {
            ...
            tasks.set(id, {
              ...ex,
              title: args.title ?? ex.title,
              status: args.status ?? ex.status,
            });
          }
        } catch { /* ignore */ }
      }
...
export default function App() {
    ...
    const handler = (event: MessageEvent) => {
      ...
      switch (message.type) {
        case 'initialState': {
          ...
          const sessionStatus: string = message.payload.status ?? 'idle';
          ...
          if (Array.isArray(message.payload.history)) {
            ...
            // Restore active permission/question if session is still waiting
            if (sessionStatus === 'permission_required') {
              for (let i = hist.length - 1; i >= 0; i--) {
                const e = hist[i];
                if (e.type === 'permission_request') {
                  setActivePermission({ action: e.action, context: e.context, requestId: e.requestId });
                  break;
                }
                if (e.type === 'ask_user_question') {
                  setActiveQuestion({ question: e.question, options: e.options, requestId: e.requestId });
                  break;
                }
              }
            }
        ...
        case 'parsed_event': {
          ...
          if (ev.type === 'permission_request') {
            setActivePermission({ action: ev.action, context: ev.context, requestId: ev.requestId });
          }
          if (ev.type === 'ask_user_question') {
            setActiveQuestion({ question: ev.question, options: ev.options, requestId: ev.requestId });
          }
          break;
        }
        case 'status_update':
  ...
  // F5: Derive tasks from history whenever it changes
  useEffect(() => {
    setTasks(deriveTasks(history));
  }, [history]);
  ...
  function handleFloatingPermissionRespond() {
    setActivePermission(null);
  }

  function handleQuestionRespond(requestId: string, answer: string) {
    setActiveQuestion(null);
    vscode.postMessage({ type: 'respondToQuestion', requestId, answer });
  }
  ...
  return (
    <div className="app">
        ...
        <div className="app-header__actions">
          ...
          {tasks.length > 0 && (
            ...
            >
              <ListChecks size={13} />
              {activeTasks > 0 && (
                <span className="task-badge">{activeTasks}</span>
              )}
            </button>
          )}
      ...
      {/* F5: app-body wraps content + task sidebar */}
      ...
      <div className="footer">
        {/* F3: Floating permission overlay */}
        {activePermission && (
          <div className="floating-overlay">
            <PermissionRequestCard
              action={activePermission.action}
              context={activePermission.context}
              requestId={activePermission.requestId}
              onRespond={handleFloatingPermissionRespond}
              variant="floating"
            />
          </div>
        )}
        {/* F4: Floating question overlay */}
        {activeQuestion && (
          <div className="floating-overlay">
            <AskUserQuestion
              ...
              onRespond={(answer) => handleQuestionRespond(activeQuestion.requestId, answer)}
            />
          </div>
        )}
        <MessageInput
